...
if(empty($_GET["bid"])&&!empty($_GET["id"])) $_GET["bid"]=$_GET["id"];
$initial=(isset($_GET['initial']))?explode("|",$_GET['initial']):array();
$act=@$_GET["act"];
$p=@$_GET["p"];
$logged=false;
$userinfo=array();
if(isset($_GET["bid"])&&strpos($_GET["bid"],"-")!==false){
	preg_match("/^(.*)-/",$_GET["bid"],$tmp);
	$_GET["bid"]=$tmp[1];
}
...
$_CH=array();
$_MENU=array();
$_PL["fxml"]="This Page contains FXML code! Open it`s in ForkPlayer or other supports FXML code browser";
...
$q=get_menu(3);
if(!empty($q["menuSets"])) $_PL["style"]["cssid"]["menu"]=$q["menuSets"];
...
if($q!=null){
	...
	foreach($a as $k=>$v){
		...
		elseif($v["title"]=="category"){
			...
			for($i=0;$i<count($plugins);$i++){
				if($do=="/plugin"&&$_GET["bid"]==$plugins[$i]["id"]) $submenu[]=["logo_30x30"=>$plugins[$i]["icon"],"search_on"=>$plugins[$i]["search_on"],"presearch"=>$plugins[$i]["presearch"],"title"=>"<b>".$plugins[$i]["name"]."</b>","playlist_url"=>$plugins[$i]["link"]];
			...
			foreach($cq as $kk=>$vv){ 
				if($do=="/category"&&$_GET["bid"]==$vv["id"]) $submenu[]=["logo_30x30"=>$vv["icon"],"title"=>"<b>".$vv["title"]." ($vv[count])</b>","playlist_url"=>""];
				else $submenu[]=["logo_30x30"=>$vv["icon"],"title"=>$vv["title"]." ($vv[count])","playlist_url"=>"$siteurl/?do=/category&bid=$vv[id]"];
			}
		...
		elseif($v["title"]=="new"){
			...
			$submenu=[];
			foreach($qp as $kk=>$vv) $submenu[]=["logo_30x30"=>$vv["icon"],"title"=>$vv["title"],"playlist_url"=>"$siteurl/?do=/fml&bid=".$vv["id"].addInf($vv["title"])];
	...
	$_MENU=array_merge($_MENU,getuserlink("mylinkonmenu"));
	
	
	foreach($a as $k=>$v){
		if($v["title"]=="roles"){
			$_MENU[]=["title"=>getmyrolesText(),"playlist_url"=>"cmd:info(<div style=\"font-weight: bold;background: #3b3e40;\">$sitename</div>Ваши роли сайта: ".getmyrolesText()."<br>Ваш box_mac: $_GET[box_mac])"];
		}
	}
}
{
	...
	if($do=="/admin"&&$logged&&$userinfo["role"]=="10") {
		...
		$link[]="/?do=/admin&act=creatormainpage|Конструктор Главной";
		...
		if($act=="addpage"){
			...
			Создать из файла XML/M3U/XSPF/FXML JSON <form action='/?do=/admin&act=addpage&op=uploadfile' enctype='multipart/form-data' method='POST'>    <input type='hidden'>    <input name='file' type='file'>     <input type='submit' value='Загрузить файл'></form><br>
			 Создать пустую страницу со всеми возможностями<br><br>
			<a href='/?do=/admin&act=addpage&op=createxml' style='border: 1px solid #ccc;    border-radius: 2px;    padding: 3px;	    text-decoration: none;    background: #f3f3f3;    color: black;'>Создать пустую страницу</a>
			";
			else{ 
				if($_GET["op"]=="uploadfile"){
					...
					elseif(strpos($f,"{")===0){
						$pf=json_decode($f,true);
					}	
					else {
						$div="<b><font color=red>Файл пустой или не содержит поддерживаемого формата страницы (XML,M3U,XSPF,FXML JSON)</font></b><hr><a href='/?do=/admin&act=addpage&op=createxml'>Создать пустую страницу</a> - страница со всеми возможностями<br><br>";						
					}
				...
				if($_GET["op"]=="delete"){
					if(mysql_query("delete from {$dbprefix}page where id=\"".mysql_real_escape_string($_GET["bid"])."\"")) $div.="Страница id #$_GET[id] удалена<hr>";
				...
				elseif($_GET["op"]=="savexml"){
					...
					foreach(explode("|","encrypt|is_iptv|sticked") as $k=>$v) {if(empty($_POST[$v])) $_POST[$v]=0;}
					if(!empty($_GET["bid"])){						
						if(($id=mysql_query("update {$dbprefix}page set category=\"".mysql_real_escape_string($category)."\",access=\"".mysql_real_escape_string($access)."\",src=\"".mysql_real_escape_string(json_encode($ch,JSON_UNESCAPED_UNICODE))."\", modified=".time().",icon=\"".mysql_real_escape_string($_POST["pltag"]["icon"])."\",title=\"".mysql_real_escape_string($_POST["pltag"]["title"])."\",seourl=\"".mysql_real_escape_string($_POST["seo"])."\",description=\"".mysql_real_escape_string($_POST["description"])."\",encrypt=\"".mysql_real_escape_string($_POST["encrypt"])."\",created=\"".mysql_real_escape_string($_POST["created"])."\",is_iptv=\"".mysql_real_escape_string($_POST["is_iptv"])."\",sticked=\"".mysql_real_escape_string($_POST["sticked"])."\" where id=\"".mysql_real_escape_string($_GET["bid"])."\""))!==false) $div.="Страница #$_GET[id] $ch[title] сохранена<hr>";
				...
				if($_GET["op"]=="createxml"){
					...
					if(empty($news_id)) $div.="Страницы $_GET[editid] нет<br>";
						...
						$div.="<form id='formxml' action='/?do=/admin&act=addpage&op=savexml&bid=$_GET[editid]' method='POST'>";
						...
						$div.="<div id=''>Ссылка на телепрограмму xmltv (в ForkPlayer поддерживаются сжатый .gz и нет) - оставьте пустым чтоб использовать авто<br><input style='width:450px;' type='text' name='pltag[url_tvg]' value='$pjson[url_tvg]' /> </div>";
		...
		if($act=="creatormainpage"){
			$q=q("select * from {$dbprefix}meta where `key`='[MAINPAGE_CREATOR]'",1);
			
			if($_GET["op"]=="save"&&isset($_POST["channels"])){
				$pf=$_POST["pltag"];
				$pf["channels"]=json_decode($_POST["channels"],true);
				$src=json_encode($pf);
				$div.="Размер страницы: ".strlen($src)."byte<br>";				
				
				if(!empty($q["id"])){
					if(mysql_query("update {$dbprefix}meta set `src`='".mysql_real_escape_string($src)."' where id=$q[id]")) $div.="Главная страница $pf[title] сохранена<hr>";
					else $div.="Ошибка сохранения страницы ".mysql_error()."<hr>";
				}
				else{
					if(mysql_query("insert into {$dbprefix}meta set uid='1',`key`='[MAINPAGE_CREATOR]', `src`=\"".mysql_real_escape_string($src)."\"")) $div.="Главная страница $pf[title] создана<hr>";
					else $div.="Ошибка создания страницы ".mysql_error()."<hr>";
				}
				$div.=mysql_error();			
			}
			else{
				if(!isset($q["id"])) $pf=["enabled"=>0];
				else $pf=json_decode($q["src"],true);
			}
			
			$div.="<form id='formxml' action='/?do=/admin&act=$act&op=save' method='POST'>";
			$div.="<input type='checkbox' ".($pf["enabled"]?" checked":"")." name='pltag[enabled]' value='1' /> Включить конструктор Главной страницы. Внимание! Это заменит вашу автоматически создаваемую главную страницу<br><br>";
			$div.="<a href='/?do=/admin&act=$act&op=generate' style='border: 1px solid #ccc;    border-radius: 2px;    padding: 3px;	    text-decoration: none;    background: #f3f3f3;    color: black;'>Сгенерировать Главную страницу</a> - в конструкторе будет создана копия вашей текущей автоматически создаваемой главной страницы<hr>";
			if($_GET["op"]=="generate"){
				$pf=json_decode(file_get_contents("$siteurl/?box_client=lg&box_mac=111111111111&op=generate"),true);
				unset($pf["style"]);
				unset($pf["menu"]);
				//print_r($pf);
			}
			...
			$pltaginfo=explode("|","Заголовок|Иконка|Вид(список,плитка)|Фоновая картинка|url|server|next_page_url|prev_page_url|access|timeout|is_iptv|all_description|pageinfo");

			for($i=0;$i<count($pltag);$i++){
				if($i<4){ 
					$div.="$pltaginfo[$i]<br>";
					if($pltag[$i]=="icon"||$pltag[$i]=="background_image") $div.="<img align='left' src='".$pf[$pltag[$i]]."' onerror=\"this.style.display='none'\" id='img_$pltag[$i]' onmouseover=\"this.width='288';this.height='162';this.style.position='absolute';this.style.zIndex='2';\"  onmouseout=\"this.width='24';this.height='20';this.style.position='relative';this.style.zIndex='';\" width=24 height=20 />";
					
					if($pltag[$i]=="typeList") $div.= "<input type='radio' checked='true' name='pltag[$pltag[$i]]' value='' /> Список <input type='radio' name='pltag[$pltag[$i]]' value='start' /> Плитка";
					else $div.="<input style='width:400px;' id='pltagid_$pltag[$i]' name='pltag[$pltag[$i]]' value='".$pf[$pltag[$i]]."' />";
					if($pltag[$i]=="icon"||$pltag[$i]=="background_image") $div.=" <input type=\"file\" name=\"file_$pltag[$i]\" id=\"file_$pltag[$i]\" alt='ico_$news_id' />"; 
					$div.="<br>";							
				}
				else continue;						
			}
			$div.="Текст вверху сайта на главной странице<br>
						<div style=\"width: 450px;background: url($sitebackground);\">
						<textarea rows=10 cols=60 id='pltagid_before' name='pltag[before]' />$pf[before]</textarea></div>";
						$div.='
						<script>
							$("#pltagid_before").htmlarea({
								css: "/include/templates/js/jHtml/jHtmlArea.Editor.css",
								toolbar: [
									["html","|","bold", "italic", "underline", "|", "forecolor"],
									["justifyLeft", "justifyCenter", "justifyRight","p"],
			...
			$div.="<input type='hidden' value='' id='pl_channels' name='channels' /><br><input type='button' value='Сохранить/Записать Главную страницу' onclick='upload_page();' />
						</form><br>";
			$div.="<div id='edit' style='width:780px;padding: 10px;position:absolute;display:none;    z-index: 1;    margin: 100px 0px 0px 30px;borrder:1px solid gray;'></div> 
				<div id='pr' style='background: url(/include/templates/images/fon20.jpg);'>
				<div id='ch' style='padding: 4px 4px 250px 4px;font-size:20px;margin-top:20px;;min-height:600px;color: $sitecolor;'></div>
			...
			if(isset($_GET["bid"])){
				...
				foreach($d as $k=>$v){
					if(is_dir(dirname(__FILE__)."/plugin/$v")&&$v!="."&&$v!=".."){
						$inf=getInfoPlugin($v);
						$div.="<input type='checkbox' name='op[$v]'".(isset($inf["enabled"])?" checked":"")." value='1' title='$inf[description]' /> <a href='/?do=/plugin&bid=$v'>$v</a> $inf[name] <a href='/?do=/admin&act=plugin&bid=$v'>Настроить</a> $inf[version]<br><i>$inf[description]</i><hr>";
					}
				}
				...
				foreach($d as $k=>$v){
					if(is_dir(dirname(__FILE__)."/module/$v")&&$v!="."&&$v!=".."){
						$inf=getInfoModule($v);
						$div.="<input type='checkbox' name='op[module$v]' disabled checked value='1' title='$inf[description]' /> <a href='/?do=/module&bid=$v&act=profile'>Модуль $v</a> $inf[name] <a href='/?do=/module&bid=$v&act=settings'>Настроить</a> $inf[version]<br><i>$inf[description]</i><hr>";
					}
				}
		...
		if($act=="menu"){
			...
			if(!isset($qe["id"])) $menuSets=["enabled"=>1];
			else $menuSets=json_decode($qe["src"],true);
			if($_GET["op"]=="save"){
				$div="";
				$ch=[];
				foreach($_POST["op"] as $k=>$v){
		...
		if($act=="sets"){
			...
			if(is_string($before[0]["src"])) $before=$_PL["before"]=$before[0]["src"];
			else $before=$_PL["before"]="";
			...
			$q=q("select * from {$dbprefix}meta where `key`='[MAINPAGE_CREATOR]'",1);
			if(isset($q["id"])){
				$a=json_decode($q["src"],true);
				if($a["enabled"]){
					$div.="<b>Внимание! У вас включен Конструктор Главной и многие настройки сайта не будут применены для Главной страницы</b><br>";
				}
			}
			$div.="
			OS: ".PHP_OS."<br>
			Версия PHP: ".phpversion()."<br>
			Версия mysql server: ".mysql_get_server_info()."<br>
			<form id='formxml' action='/?do=/admin&act=sets&op=save' method='POST'>
				Название
				<br>
				...
				<input type='radio' ".($typelinkPage==''?" checked":"")." name='pltag[typelinkPage]' value='' /> /?do=/fml&bid=<b>ID-TITLE</b> (рекомендуется для индексации поисковой системой Spider)<br> 
				<input type='radio' name='pltag[typelinkPage]' ".($typelinkPage=='id'?" checked":"")." value='id' /> /?do=/fml&bid=<b>ID</b> <br>
		...
		if($act=="adddevice"){
			...
			elseif($_GET["is_user"]||isset($_POST["op"]["email"])){
				if($_GET["op"]=="delete"){
					if(mysql_query("delete from {$dbprefix}users where id=\"".mysql_real_escape_string($_GET["bid"])."\"")) $div.="Пользователь id #$_GET[id] удален<hr>";
					else $div.="Такой пользователь с id #$_GET[id] не найден ".mysql_error()."";
				} 
				elseif($_GET["op"]=="save"){
					
					mysql_query("ALTER TABLE {$dbprefix}users ADD meta TEXT");	
					$dd=0;
					...
					deletePluginMetaKey("[MYMENU_$_GET[id]]","*");
						
					$access=",";
					if(preg_match("/[^a-z0-9_\.]/i",$_POST["op"]["login"])&&empty($_GET["bid"]))  $err.="Ошибка!<br><span style='color:red;'>Логин может содержать только латинницу, _ и . (".$_POST["op"]["login"]."!)</span><br>";					
					...
					elseif($_POST["op"]["pass1"]!=$_POST["op"]["pass1"]) $div.="Ошибка! Пароли не совпадают<br>";
						...
						if(($id=mysql_query("update {$dbprefix}users set login=\"".mysql_real_escape_string($_POST["op"]["login"])."\",`email`=\"".mysql_real_escape_string($_POST["op"]["email"])."\",`mac`=\"".mysql_real_escape_string($_POST["op"]["mac"])."\"$setp$d,meta='".mysql_real_escape_string(json_encode($MYCH))."' where id=\"".mysql_real_escape_string($_GET["bid"])."\""))!==false) $div.="Пользователь ".$_GET["bid"]." сохранен<br>";
						else $div.="Ошибка! ".mysql_error()."<br>";
					}
					else{
						if(strlen($_POST["op"]["pass1"])<4) $div.="Ошибка! Пароль должен иметь длину не меньше 4!<br>";
						elseif(register_user($_POST["op"]["login"],$_POST["op"]["email"],$_POST["op"]["pass1"],$_POST["op"]["mac"],$_POST["op"]["role"],$dd,json_encode($MYCH))) $div.="Пользователь ".$_POST["op"]["login"]." добавлен<br>";
						else $div.="Ошибка! ".mysql_error()."<br>";
						
					}					
					$_GET["editid"]=$_GET["bid"];
				}
    ...
    background: #f7f7f72e;'>Вернуться в список пользователей и устройств</a>  <hr>";
				...
				if($_GET["op"]!="delete"){				
					if(isset($_GET["editid"])){
							...
							if(!empty($q["meta"])) $MYCH=json_decode($q["meta"],true);
							else{
					...
					if(isset($_GET["editid"])&&!isset($q[0])) $div.="Такого пользователя нет или вы его только создали<br>";
						...
						<form id='formxml' action='/?do=/admin&act=$act&op=save&bid=$_GET[editid]&is_user=$_GET[is_user]' method='POST'>
						Логин<br>
			...
			<datalist id="term">
				...
				mysql_query("ALTER TABLE {$dbprefix}device ADD meta TEXT");	
				...
				if($_GET["op"]=="addfile"){
					...
					if($_GET["op"]=="delete"){
						...
						else mysql_query("delete from axml_device where mac='".mysql_real_escape_string($_GET["mac"])."' and id='".mysql_real_escape_string($_GET["bid"])."'");
					...
					if($_GET["op"]=="add"){
						...
						elseif(strpos($_POST["dateto"],".")>0) {
							$d2=strtotime($_POST["dateto"]);
							$div.="Доступ до даты: ".date("d.m.Y",$d2)." <br>";
							$d=",dateto='".mysql_real_escape_string($d2)."'";
						}
						elseif(intval($_POST["dateto"])>0) {
							$div.="Доступ до даты: ".date("d.m.Y",time()+intval($_POST["dateto"])*24*3600)." <br>";
							$d=",dateto='".mysql_real_escape_string(time()+intval($_POST["dateto"])*24*3600)."'";
						}
						else $d="";
						$MYCH=["mylinkonmain"=>$_POST["mylinkonmain"]?1:0,"mylinkonmenu"=>$_POST["mylinkonmenu"]?1:0,"channels"=>[]];
						foreach($_POST["mych"] as $v){
						...
						else $mq=q("select * from {$dbprefix}device where mac='".mysql_real_escape_string($_POST["mac"])."' and role='".mysql_real_escape_string($_POST["roleid"])."'",1);
						if(!isset($mq[0])) {
							if(mysql_query("insert into {$dbprefix}device set mac='".mysql_real_escape_string($_POST["mac"])."'$d,role='".mysql_real_escape_string($_POST["roleid"])."',about='".mysql_real_escape_string($_POST["about"])."',meta='".mysql_real_escape_string(json_encode($MYCH))."',modified=NOW()")) $div.="Идентификатор $_POST[mac] еще ниразу не входил на сайт, добавлен в ".get_role($_POST["roleid"])."<br>";
							else print "Ошибка ".mysql_error()."<br>";
							$mq=q("select id from {$dbprefix}device where mac='".mysql_real_escape_string($_POST["mac"])."' and role='".mysql_real_escape_string($_POST["roleid"])."'",1);
							$_GET["editid"]=$mq["id"];
						}
						else{
							if(!empty($mq["userid"])) $mu=q("select * from {$dbprefix}users where id='".mysql_real_escape_string($mq["userid"])."'",1);
							if(mysql_query("update {$dbprefix}device set role='".mysql_real_escape_string($_POST["roleid"])."'$d,about='".mysql_real_escape_string($_POST["about"])."',meta='".mysql_real_escape_string(json_encode($MYCH))."',modified=NOW() where id='$mq[id]'")) $div.="Идентификатор $_POST[mac] добавлен в ".get_role($_POST["roleid"])."<br>Информация: ".((!empty($mq["ip"]))?"IP: ".$mq["ip"]." Вход на сайт ".date("d.m.Y H:i",$mq["last"]):"Не входил на сайт")."   <br>".((isset($mu["login"]))?" Логин: $mu[login] Email: $mu[email]":"");
						}
	...
	<div style='border:1px solid gray;border-radius:4px;padding:4px;'>
			<form id='formxml' action='/?do=/admin&act=$act&op=add&editid=$_GET[editid]&bid=$_GET[editid]&is_user=$_GET[is_user]' method='POST'>
			<table>
			<tr><td>Мак или ForkPlayerID<br><input id='mac' name='mac' value='$q[mac]' /></td>
			...
			<input id="dateto" name="dateto" list="term" value="'.($q["dateto"]>0?date("d.m.Y",$q["dateto"]):$q["dateto"]).'" />
			<datalist id="term">
				<option value="0">Навсегда</option>
				<option value="03">3 дня</option>
				<option value="30">30 дней</option>
    ...
    '."</td><td><br>
	<input type='button' onclick=\"javascript:if(\$('#mac').val().match(/[a-z0-9]{12,16}/)||\$('#mac').val().match(/\S+@\S+\.\S+/)) \$('#formxml').submit(); else confirm('Мак адрес должен состоять из 12-16 символов латинницей и цифр в нижнем регистре без двоеточий. \\nИли ForkPlayerID должен быть в виде email');\" value='".(isset($_GET["editid"])?"Сохранить #$_GET[editid]":"Добавить новое устройство")."' /></table>";
	$MYCH=json_decode($q["meta"],true);
		//if(isset($_GET["editid"])){
			$div.="Личные ссылки устройства<br>
			<div style='border:1px solid gray;border-radius:5px;'>
			<input type='checkbox'".($MYCH["mylinkonmain"]?" checked":"")." name='mylinkonmain' value='1' /> Отобразить на главной сайта<br>
			<input type='checkbox'".($MYCH["mylinkonmenu"]?" checked":"")." name='mylinkonmenu' value='1' /> Отобразить в меню сайта<br>
			<table><tr><td>URL</td><td>TITLE</td><td>ICON</td></tr>";
...
if($act=="devices"){
	...
	if(empty($qc)) $qc=20;
			mysql_query("ALTER TABLE `{$dbprefix}users` CHANGE `mac` `mac` VARCHAR( 72 ) NOT NULL DEFAULT ''");
...
group by role");
	...
	if(!empty($_POST["search"]))  {
		...
		$dpo="where (about like '%$search%' or ip like '%$search%' or login like '%$search%' or mac like '%$search%' or initial like '%$search%') $dpo";
	}
	elseif(!empty($_GET["roleid"])) {
		if($_GET["roleid"]==4) $dpo="where (is_user='1') $dpo";
			...
			$q="select * FROM (
	(select id, mac as login, mac, role, dateto, ip,  last, modified, about, initial, 0 as is_user, c from {$dbprefix}device) 
	union all
	(
		SELECT * FROM 
		(
			SELECT
			u.id, u.login as login, u.mac as mac, u.role, u.dateto,s.ip,s.last, u.modified,  CONCAT(u.email, '~|~', u.forkplayerid) as about, s.initial, 1 as is_user, s.c as c
...
$div.=mysql_error()." ";
	...
	<table>
	<tr><td><a href='/?do=/admin&act=$act&roleid=".$_GET["roleid"]."&order=id'>ID ред.</a></td><td>_____<a href='/?do=/admin&act=$act&roleid=".$_GET["roleid"]."'>Обновить</a>_____</td><td><a href='/?do=/admin&act=$act&roleid=".$_GET["roleid"]."&order=dateto'>Срок</a></td><td>Роль</td><td>Примечание</td><td>Изменено</td><td><a href='/?do=/admin&act=$act&roleid=".$_GET["roleid"]."&order=last'>Последний вход</a></td><td>IP</td><td><a href='/?do=/admin&act=$act&roleid=".$_GET["roleid"]."&order=c'>Входов</a></td></tr>";
		if(count($ch)<1) $div.="<tr><td>Нет.</td></tr>";  
			for($i=0;$i<count($ch);$i++){
				$div.="<tr onmouseover=\"this.style.backgroundColor='#808080a1';\" onmouseout=\"this.style.backgroundColor='';\" ><td><a href='$siteurl/?do=/admin&act=adddevice&editid=".$ch[$i]["id"]."&is_user=".$ch[$i]["is_user"]."'>".$ch[$i]["id"]."</a></td>
				
				<td title=\"".$ch[$i]["initial"]."\" style='max-width:220px;word-break: break-all;'>".(($ch[$i]["login"]!=$ch[$i]["mac"])?"<b>".$ch[$i]["login"]."</b><br>":"")."".$ch[$i]["mac"]."</td>
				
				<td style='".(($ch[$i]["dateto"]>0&&$ch[$i]["dateto"]<time())?"color:red;":"")."'> ".(($ch[$i]["dateto"]=="0")?"Безсрочно":date("d.m.Y",$ch[$i]["dateto"]))."</td>
				
				<td style='max-width:150px;'> ".(($ch[$i]["is_user"]&&$ch[$i]["role"]!=4)?get_role(4).", ".get_role($ch[$i]["role"]):get_role($ch[$i]["role"]))."</td>
				
				<td style='max-width:200px;word-break: break-all;'> ".$ch[$i]["about"]."</td><td> ".$ch[$i]["modified"]."</td><td> ".(($ch[$i]["last"]<1)?"Не входил":date("d.m.Y H:i",$ch[$i]["last"]))."</td>

				<td> ".$ch[$i]["ip"]."</td><td> ".$ch[$i]["c"]."</td><td><a href='/?do=/admin&act=adddevice&op=delete&roleid=".$ch[$i]["role"]."&mac=".$ch[$i]["mac"]."&bid=".$ch[$i]["id"]."&is_user=".$ch[$i]["is_user"]."'>Удалить</a></td>";
				$div.="</tr>";
			}
		...
		if($act=="addcat"){
			...
			if($_GET["op"]=="delete"){
				if(mysql_query("delete from {$dbprefix}category where id=\"".mysql_real_escape_string($_GET["bid"])."\"")) $div.="Категория #".$_GET["bid"]." удалена!";
			...
			elseif($_GET["op"]=="save"){
				...
				for($i=0;$i<count($pq);$i++){
					...
					<form action='/?do=/admin&act=addcat&op=save&bid=$_GET[editid]' method='POST'>
					Название<br>
		...
		if($act=="cats"){
			...
			function desCat($tmp){
				...
				<a href=\"javascript:if(confirm('Вы уверены что хотите удалить категорию ".$tmp["title"]."?')) location='/?do=/admin&act=addcat&op=delete&bid=".$tmp["id"]."';\">удалить</a><br>";
			}
			foreach ($ch as $k=>$v) { //Обходим массив
				if($v["parent"]==$v["id"]) $v["parent"]=0;
	...
	elseif($do=="/fml"){
		if(!empty($_GET["bid"]))	$page=get_page($_GET["bid"]);
		...
		if(isset($_GET["proxylink"])){
			$prl=0;
			if($_GET["lev"]==1){
				if(md5($secret.$ip.$_GET["box_mac"].$_GET["lev"].$_SERVER['HTTP_USER_AGENT'])==$_GET["hash"]) $prl=1;
			}
			elseif(md5($secret.$ip.$_GET["lev"].$_SERVER['HTTP_USER_AGENT'])==$_GET["hash"]) $prl=1;
			
			if($prl){
				$s=file_get_contents($a["channels"][$_GET["proxylink"]]["playlist_url"]);
				print listEncrypt($s,$_GET["lev"]);
			}
			else print "Ошибка доступа по защищенной ссылке! Возможно у вас поменялся внешний IP, box_mac или устройство. Перезапустите браузер";
			exit;
		} 
		...
		for($i=0;$i<count($a["channels"]);$i++){
			if($a["channels"][$i]["playlist_url"]!=""){
				$proxyurl=$a["channels"][$i]["proxyurl"];
				if($proxyurl=="1") $a["channels"][$i]["playlist_url"]="$siteurl/?do=/fml&proxylink=$i&lev=".$proxyurl."&bid=$_GET[id]".addInf($a["channels"][$i]["title"])."&hash=".md5($secret.$ip.$_GET["box_mac"].$proxyurl.$_SERVER['HTTP_USER_AGENT']);
				elseif($proxyurl=="2"||$proxyurl=="3"||$proxyurl=="4") $a["channels"][$i]["playlist_url"]="$siteurl/?do=/fml&proxylink=$i&lev=".$proxyurl."&bid=$_GET[id]".addInf($a["channels"][$i]["title"])."&hash=".md5($secret.$ip.$proxyurl.$_SERVER['HTTP_USER_AGENT']);
			}
		}
		$_CH=$a["channels"];
	...
	elseif(empty($do)||$do=="/"||$do=="/category"){
		...
		if(empty($do)||$do=="/") {
			...
			$before=getPluginMetaKey("[MAINBEFORE]",false);
			if(is_string($before[0]["src"])) $_PL["before"]=$before[0]["src"];
		}
		elseif($do=="/category") {
			$qc=q("select * from {$dbprefix}category where id='".mysql_real_escape_string($_GET["bid"])."'",1);
		...
		for($i=0;$i<count($plugins);$i++){
			if(!$_ISPC) {
				$tmp=[];
				foreach(explode("|",$chtag) as $k=>$v) if(!empty($plugins[$i][$v])) $tmp[$v]=$plugins[$i][$v];
				$_CH[]=$tmp;
			}
		...
		if($_ISPC){
			...
			function outTree($parent, $level) {
				...
				if (isset($_getCategory[$parent])) { //Если категория с таким parent_id существует
					foreach ($_getCategory[$parent] as $value) { //Обходим ее
						...
						if($_GET["bid"]==$value["id"]) $lmenu.="<b>".$value["title"]."</b> (".$value["count"].")<br>";
						else $lmenu.="<a href='/?do=/category&bid=".$value["id"]."'>".$value["title"]."</a>(".$value["count"].")<br>";
						$lmenu.= "</div>";
						$level++; //Увеличиваем уровень вложености
						//Рекурсивно вызываем этот же метод, но с новым $parent_id и $level
						outTree($value["id"], $level);
			...
			for($i=0;$i<count($ch);$i++){
				$_CH[]=["logo_30x30"=>$ch[$i]["icon"],"playlist_url"=>"$siteurl/?do=/category&bid=".$ch[$i]["id"]."-".toTranslit($ch[$i]["title"]),"title"=>($_GET["bid"]==$ch[$i]["id"]?"<b>".$ch[$i]["title"]."</b>":$ch[$i]["title"])."(".$ch[$i]["count"].")"];
		...
		$pages=get_pages($_GET["bid"]);

		
		if(!empty($_GET["bid"])) {
			$cq=q("select * from {$dbprefix}category where parent='".mysql_real_escape_string($_GET["bid"])."'");

			for($i=0;$i<count($cq);$i++){
				$el=["icon"=>$cq[$i]["icon"],"playlist_url"=>"$siteurl/?do=/category&bid=".$cq[$i]["id"]."-".toTranslit($cq[$i]["title"]),"title"=>($_GET["bid"]==$cq[$i]["id"]?"<b>".$cq[$i]["title"]."</b>":$cq[$i]["title"])."(".$cq[$i]["count"].")","description"=>"Категория<br>".$cq[$i]["description"],"category"=>$cq[$i]["parent"]];
				if(count($pages))  array_unshift($pages,$el);
				else $pages[]=$el;
				
			}
			$TITLE=get_cat_roles_by_id($_GET["bid"],false)." ";
		}
		...
		for($i=0;$i<count($pages)&&$i<$siteperpage;$i++){
			...
			$link=(isset($pages[$i]["playlist_url"])?$pages[$i]["playlist_url"]:"$siteurl/?do=/fml&bid=".$pages[$i]["id"].addInf($pages[$i]["title"]));
			if(!$_ISPC) {
	...
	xhr.send();
		...
		if(empty($do)||$do=="/"){
			...
			if(isset($q["id"])){
				...
				if($a["enabled"]){
					$oldP=$_PL;
					$_PL=array_merge(["fxml"=>$oldP["fxml"]],$a);
					$_PL["style"]=$oldP["style"];
					$_PL["navigate"]=$_PL["title"];				 
				}
			}
		}	
		$_PL["menu"]=$_MENU;		
		if(empty($do)||$do=="/") $_PL["channels"]=array_merge($_PL["channels"],getuserlink("mylinkonmain"));
		//$_PL["before"].="<style>@import url('https://fonts.googleapis.com/css?family=PT+Sans&display=swap');</style>";
		//$_PL["style"]["cssid"]["site"]["fontFamily"]="'PT Sans', sans-serif"; //Имя шрифта
		$_PL["before"].=$BEFORE;
		if($showStat&&$_GET["op"]!="generate"){
		...
		if($_GET["box_client"]=="lg"||strpos($_GET["box_client"],"OTTPlayer")!==false) print json_encode($_PL);
		else{
			//print_r($_PL);
			print cxmljson($_PL);
		}
