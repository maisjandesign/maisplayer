...
function register_user($login,$email,$password,$mac="",$role=4,$dateto=0,$meta=""){
	global $dbprefix,$secret;
	if($role<1) $role=4;
	
	mysql_query("insert into {$dbprefix}users set login='".mysql_real_escape_string($login)."', email='".mysql_real_escape_string($email)."', password='".mysql_real_escape_string(md5($secret.$password))."', mac='".mysql_real_escape_string($mac)."', `role`='".intval($role)."', dateto='".intval($dateto)."', `meta`='".mysql_real_escape_string($meta)."'");
	$pe=mysql_error();
	if(!empty($pe)) exit("Ошибка при регистрации: ".$pe);
...
function get_search($text){
	global $dbprefix,$ROLE,$logged,$userinfo,$initial,$_ISPC,$siteurl,$PLUGIN,$BEFORE;
	...
	$BEFORE.="<div style=''>Результаты поиска по запросу $text: </div>";
	...
	for($i=0;$i<count($qmb);$i++){
		...
		foreach($tmp["channels"] as $k=>$v){
			if(mb_stripos($v["title"],$text,null,'UTF-8')!==false){
				$v["menu"][]=["logo_30x30"=>$tmp["icon"],"title"=>"На страницу ".$tmp["title"],"playlist_url"=>"$siteurl/?do=/fml&bid=".$qmb[$i]["id"]];
	...
	for($i=0;$i<count($q);$i++){
		$r[]=["logo_30x30"=>$q[$i]["icon"],"title"=>$q[$i]["title"],"playlist_url"=>"$siteurl/?do=/category&bid=".$q[$i]["id"],"description"=>"Найдено в категориях сайта"];
	}	
	$ide=",";
	for($i=0;$i<count($q2);$i++){
		$r[]=["logo_30x30"=>$q2[$i]["icon"],"title"=>$q2[$i]["title"],"playlist_url"=>"$siteurl/?do=/fml&bid=".$q2[$i]["id"],"description"=>"Найдено в страницах сайта по названию<br>".$q2[$i]["description"]];
		$ide.=$q2[$i]["id"].",";
	}
	for($i=0;$i<count($q3);$i++){
		if(strpos($ide,",".$q3[$i]["id"].",")!==false) continue;
		$r[]=["logo_30x30"=>$q3[$i]["icon"],"title"=>$q3[$i]["title"],"playlist_url"=>"$siteurl/?do=/fml&bid=".$q3[$i]["id"],"description"=>"Найдено в страницах сайта по описанию<br>".$q3[$i]["description"]];
		$ide.=$q3[$i]["id"].",";
	}
	
	foreach($PLUGIN["settings"]["pl"] as $k=>$v){
		//print_r($v);
		if(is_dir(dirname(__FILE__)."/plugin/$k")){
			$inf=getInfoPlugin($k);
			if($v["enabled"]){
				$r[]=["logo_30x30"=>$inf["icon"],"title"=>"Искать в ".$inf["title"],"iframe"=>"4","playlist_url"=>str_replace("%s",urlencode($text),$v["url"]),"description"=>"Iframe поиск"];
			}
		}
	}
...
function get_page($id){
	global $_PL,$dbprefix,$ROLE,$logged,$userinfo,$initial,$_ISPC,$sitename,$siteurl;
	...
	//print_r($_PL);
	$q["src"]='{"channels":[{"title":"У вас недостаточно прав для просмотра этой страницы!","description":"'.$sitename.'<br>У вас недостаточно прав для просмотра этой страницы!<br> (C) '.$siteurl.'<br>'.$q["description"].'"}]}';
	return $q;
}
...
function getArrayAvailableUserRoles(){
	...
	$ac=[0,1,2,3];
	$utemp=$userinfo;
	if(!$logged&&!empty($_GET["box_mac"])) {
		$qu=q("select * from {$dbprefix}users where mac like '%".mysql_real_escape_string("$_GET[box_mac]")."%'",1);
		if(isset($qu["login"])){
			$utemp=$qu;
			$userinfo["macid"]=$qu["id"];
		}
	}	
	$userinfo["macmeta"]=[];	
	if($logged||isset($qu["login"])){
		$ac[]=4;
		if($utemp["dateto"]<1||$utemp["dateto"]>time()) {
			if($utemp["role"]<3) $ac=[$utemp["role"]];
			elseif($utemp["role"]==9){$ac[]=5;$ac[]=6;$ac[]=7;$ac[]=8;$ac[]=9;}
			elseif($utemp["role"]==10){$ac[]=5;$ac[]=6;$ac[]=7;$ac[]=8;$ac[]=9;$ac[]=10;}
			else $ac[]=$utemp["role"];
			if($utemp["role"]>4&&$utemp["dateto"]>0)	$roleTo[$utemp["role"]]=$utemp["dateto"];
		}
		elseif($utemp["role"]>4)	$roleEstimated[$utemp["role"]]=$utemp["dateto"];
		if(!$logged&&!empty($utemp["meta"])){
			$userinfo["macmeta"][]=json_decode($utemp["meta"],true);
		}
		//print $utemp["dateto"].">".time();print_r($ac);
		mysql_query("update {$dbprefix}sessions set ip='".mysql_real_escape_string($ip)."',last=".time().",initial='".mysql_real_escape_string($_GET["initial"])."',c=c+1 where sid='$utemp[sid]'");
		if(!empty($utemp["forkplayerid"])) {
			$qd=q("select * from {$dbprefix}device where mac='".mysql_real_escape_string($utemp["forkplayerid"])."'",1);			
			if(isset($qd[0])){
		...
		if(!empty($utemp["login"])) {
			$qd=q("select * from {$dbprefix}device where mac='".mysql_real_escape_string($utemp["login"])."'",1);
			if(isset($qd[0])){
				mysql_query("update {$dbprefix}device set ip='".mysql_real_escape_string($ip)."',last=".time().",initial='".mysql_real_escape_string($_GET["initial"])."',c=c+1 where id=$qd[id]");
				if($qd["dateto"]<1||$qd["dateto"]>time()) {
					if($qd["role"]<3) $ac=[$qd["role"]];
	...
	if(!empty($_GET["box_mac"])) {
		...
		for($i=0;$i<count($qd);$i++){
			if(!empty($qd[$i]["meta"])) $userinfo["macmeta"][]=json_decode($qd[$i]["meta"],true);	
...
function getPluginMetaKey($k,$json=false,$id="",$name=""){
	global $dbprefix,$MODULE,$PLUGIN;
	if(!empty($name)) $mid=$name;
	elseif(isset($MODULE["id"])) $mid=$MODULE["id"];
	else $mid=$PLUGIN["id"];
...
function deletePluginMetaKey($k,$id=null){
	...
	print "delete from {$dbprefix}meta where `key`='".mysql_real_escape_string("PluginMeta_{$mid}_$k")."' and id='".mysql_real_escape_string("$id")."'";
	if(preg_match("/[^a-z_0-9]/i",$PLUGIN["id"])) return ["name"=>"Error name of plugin"];
	if($id=="*") {
		if(mysql_query("delete from {$dbprefix}meta where `key`='".mysql_real_escape_string("PluginMeta_{$mid}_$k")."'")) return true;
	}
	else if(mysql_query("delete from {$dbprefix}meta where `key`='".mysql_real_escape_string("PluginMeta_{$mid}_$k")."' and id='".mysql_real_escape_string("$id")."'")) return true;
	else return false;
}
...
function usorP($a, $b) {	
	return intval($a["settings"]["order"])>intval($b["settings"]["order"]);
}
...
function getInfoPlugin($v){
	...
	if($a["playlist_url"]==null) $a["playlist_url"]="";
	if(!empty($a["presearch"])) $a["presearch"]="$siteurl/?do=/plugin&bid=$v&$a[presearch]";
	$a["path"]="$siteurl/include/plugin/$v";
	$a["link"]="$siteurl/?do=/plugin&bid=$v-".toTranslit($a["name"]);
	$a["title"]=$a["name"];
	if(strpos($a["playlist_url"],"http")!==0) $a["playlist_url"]=$a["link"].$a["playlist_url"];
	//print_r($a);
	return $a;
}
function getInfoModule($v){
	...
	$a["link"]="$siteurl/?do=/module&bid=$v";
	return $a;
}
function get_menu($m=false){
	...
	if(!isset($q["id"])) $menuSets=["enabled"=>1];
	else $menuSets=json_decode($q["src"],true); 
	...
	if(!isset($q["id"])) {
		...
		if($m) {
			if($m==3) $modules=getModules("menu");
			else if($m==2) $modules=getModules();
			else $modules=getModules("main");
...
function get_role($id="get"){
	...
	if(count($q)<1) {
		mysql_query("INSERT INTO `{$dbprefix}role` (`id`, `name`) VALUES (1, 'Заблокирован'),(2, 'Читатель'),(3, 'Гость'),(4, 'Пользователь'),(5, 'Важный пользователь'),(6, 'Группа 1'),(7, 'Группа 2'),(8, 'Группа 3'),(9, 'Модератор'),(10, 'Администратор');");
		print mysql_error();
		$q=q("select * from {$dbprefix}role");		
	}
...
function chToHtml($ch,$template){
	...
	if(!empty($ch["search_on"])){
		...
		if(strpos($ch["playlist_url"],"payd_")===0) {
			...
			$ch["playlist_url"]="#";
			$ch["description"].="$befor<input type='$ch[type]'".($ch["search_on"]=="checked"?$ch["search_on"]:"")." id='$name' name='$name' value='".$ch["value"]."' size=80 /><br><small></small>";
		}
		else {
			$name="search";
...
$fE4=1;
function fEncrypt($s){
	global $initial,$ip,$siteurl,$fE4,$secret;
	...
	if($_GET["lev"]==4) return $s;
	elseif($_GET["lev"]==3)	{
		if(isset($_GET["live"])){
			if($_GET["live"]==$fE4){
				if(md5($secret.$fE4.$ip)==$_GET["livehash"]) header("Location: $s");
				else header('HTTP/1.0 403 Forbidden');
				exit;
			}
		}
		$l="&live=$fE4&livehash=".md5($secret.$fE4.$ip);
		$fE4++;
		if(strpos($s,".m3u8")>0) $l.="&.m3u8";
		return "$siteurl".$_SERVER['REQUEST_URI']."$l";
	}
	elseif($_GET["lev"]==2)	$s.="-9-$ip-9-".time()."-9-";
	else $s.="-9-$ip-9-".time()."-9-$_GET[box_mac]";
	$l=base64_encode($s);
...
$chtag="title|logo_30x30|stream_url|playlist_url|description|category|parser|mb_parser|local|after|before|menu_url|search_on|presearch|jtvname|submenu|tvg-shift|value|group|subtitles|yellow_url|yellow_title|yellow_parser|infolink|location|advert|type|menu|position";
...
function getuserlink($kk){ 
	global $logged, $userinfo,$_ISPC;
	if($logged){
		if(!empty($userinfo["meta"])) $MYCH=json_decode($userinfo["meta"],true);
		$MYCH2=getPluginMetaKey("[MYMENU_$userinfo[id]]",true,null,"auth");
		$MYCH2=$MYCH2[0]["src"];
	}
	elseif(!empty($userinfo["macid"])){
		$MYCH2=getPluginMetaKey("[MYMENU_$userinfo[macid]]",true,null,"auth");
		$MYCH2=$MYCH2[0]["src"];
	}
	
	$submenu=[];
	foreach($userinfo["macmeta"] as $k=>$v){
		if($v[$kk]) $submenu=array_merge($submenu,$v["channels"]);
	}
	if($MYCH[$kk]) $submenu=array_merge($submenu,$MYCH["channels"]);
	if($MYCH2[$kk]) $submenu=array_merge($submenu,$MYCH2["channels"]);
	
	if(count($submenu)==1||(count($submenu)&&$_GET["box_client"]!="lg"&&!$_ISPC)) return $submenu;
	elseif(count($submenu)>1) return [["title"=>"Мои ссылки","playlist_url"=>"submenu","submenu"=>$submenu]];	
	else return [];	
}
